...
class World():
    ...
    def get_valid_acts(self, s):
        ...
        ##it returns all valid actions of s
...
class Agent():
    ...
    def make_move(self, s, valid_moves):
         ##remove the following statement
        ##implement eplison greedy to determine the next action
        ##Hint: call random.choice() to randomly pick an action
        act = rl_helper.make_move(self, s, valid_moves)
        
        return act

    def update_Q(self, s, a, next_s, this_reward):
        ##remove the following statement
        ##update the q, rember to use self.set_Q()
        rl_helper.update_Q(self, s, a, next_s, this_reward)
