...
try:
    ...
    df = pd.read_csv(r'D:\\My Drive\\NYU Main\\NYU Y1S1\\ICS Section 002\\Lec Slides\\Week 9\\benford_law\\president_county_candidate.csv')
    vote_records = []
    for i in range(len(df.candidate)):
        vote_records.append(list(df.iloc[i, :]))
    f = open(r'D:\\My Drive\\NYU Main\\NYU Y1S1\\ICS Section 002\\Lec Slides\\Week 9\\benford_law\\US_election_2020.pk', 'wb')
